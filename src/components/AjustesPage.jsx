...
import { PROMO_SPREADSHEET_ID } from "../services/googleSheetsService";
...
export default function AjustesPage({
  ...
  return <div>
    <Header title="Ajustes" subtitle="Configuracion administrativa de catalogos, conexion Drive, notificaciones y fuentes maestras." />
    <div className="settings-layout">
      <div className="settings-stack">
        <Card>
          <CardContent>
            <div className="toolbar">
              <h2>Catalogos</h2>
              <Button variant="outline" onClick={addCatalogo}><Plus size={16}/> Nuevo</Button>
            </div>
            <div className="list">
        ...
        <Card className="buyers-card">
          <CardContent>
            <div className="toolbar">
              <h2>Compradores</h2>
              <div className="toolbar-actions">
                <Button variant="outline" onClick={() => onSaveCatalogSettings({ catalogos, compradores })} disabled={isSyncing}><Save size={16}/> Guardar</Button>
                <Button variant="outline" onClick={addComprador}><Plus size={16}/> Nuevo</Button>
              </div>
            </div>
            <div className="buyer-manager">
              ...
              {selectedBuyer ? <div className="buyer-form">
                ...
                <div className="button-row">
                  <Button variant="outline" onClick={toggleComprador}>{selectedBuyerActive ? "Inactivar" : "Activar"}</Button>
                  <Button variant="outline" onClick={deleteComprador}><Trash2 size={16}/> Quitar</Button>
                </div>
      ...
      <Card>
        <CardContent>
          <div className="toolbar">
            ...
            <div className="toolbar-actions">
              <Button onClick={() => onSaveCatalogSettings({ catalogos, compradores })} disabled={isSyncing || !selected}><Save size={16}/> Guardar ajustes</Button>
              <Button variant="outline" onClick={deleteSelected} disabled={isSyncing || !selected || catalogos.length <= 1}><Trash2 size={16}/> Borrar</Button>
            </div>
          </div>
          <div className="form-grid">
            <div className="connection-panel wide">
              <div>
                <strong>Conexion Google Sheets</strong>
                <span>Hoja {PROMO_SPREADSHEET_ID}</span>
                <p>{driveStatus?.message || "Pegue aqui la URL /exec publicada desde Apps Script."}</p>
              </div>
              <label className="field wide">
                <span>URL Apps Script</span>
                <input value={driveConnection.webAppUrl || ""} onChange={(e) => updateDrive("webAppUrl", e.target.value)} placeholder="https://script.google.com/macros/s/.../exec" />
              </label>
              <label className="field">
                <span>Token API</span>
                <input type="password" value={driveConnection.token || ""} onChange={(e) => updateDrive("token", e.target.value)} placeholder="Opcional" />
              </label>
              <label className="field">
                <span>ID Google Sheets</span>
                <input value={PROMO_SPREADSHEET_ID} readOnly />
              </label>
              <div className="button-row wide">
                <Button onClick={onSaveDriveSettings} disabled={isSyncing}><Save size={16}/> Guardar conexion</Button>
                <Button variant="outline" onClick={onTestDriveConnection} disabled={isSyncing}><Search size={16}/> Probar</Button>
                <Button variant="outline" onClick={onSetupDriveWorkbook} disabled={isSyncing}><FileSpreadsheet size={16}/> Preparar hojas</Button>
              </div>
            </div>
