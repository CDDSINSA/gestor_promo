      ...
      <Card className="special-request-detail-card">
        <CardContent>
          <div className="section-head">
            ...
            <div className="toolbar-actions">
              {selected && <Button variant="outline" onClick={() => setActive("especial")}><Plus size={16}/> Nueva especial</Button>}
              <Button onClick={onSaveDrive} disabled={!driveReady || isSyncing}><Save size={16}/> {saveDriveLabel}</Button>
            </div>
          </div>
          {selected ? <div className="special-detail-grid">
            <label className="field"><span>Estado</span><select value={selectedStatus} onChange={(e) => requestStatusChange(selected, e.target.value)}>{SPECIAL_REQUEST_STATUSES.map((status) => <option key={status}>{status}</option>)}</select></label>
            <label className="field"><span>Responsable</span><select value={selected.responsable || ""} onChange={(e) => updateField(selected, "responsable", e.target.value)}><option value="">Sin asignar</option>{selected.responsable && !responsables.some((item) => item.nombre === selected.responsable) && <option value={selected.responsable}>{selected.responsable} - no esta en catalogo</option>}{responsables.map((item) => <option key={item.responsable_id || item.nombre} value={item.nombre}>{item.area ? `${item.nombre} - ${item.area}` : item.nombre}</option>)}</select></label>
            ...
            <label className="field wide"><span>Recursos ocupados</span><textarea value={selected.recursos_ocupados || ""} onChange={(e) => updateField(selected, "recursos_ocupados", e.target.value)} placeholder="Ej. pricing, pauta digital, diseno, rotulacion" /></label>
        ...
        return <section key={status} className={classNames("special-board-column", `status-${key}`)}>
          ...
          <div className="special-board-list">
            {items.map((item) => <article key={item.actividad_id} className={classNames("special-task-card", selected?.actividad_id === item.actividad_id && "selected")} onClick={() => setSelectedId(item.actividad_id)}>
              ...
              {nextStatus && <Button variant="outline" onClick={(event) => { event.stopPropagation(); requestStatusChange(item, nextStatus); }}><ArrowRight size={14}/> {nextStatus}</Button>}
            </article>)}
