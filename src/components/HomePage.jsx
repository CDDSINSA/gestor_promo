...
export default function HomePage({ catalogos, rows = [], rowsCount, logsCount, setActive, setCatalogoActivo, onOpenAvances, onLoadExcel, onSaveExcel, onLoadDrive, onSaveDrive, driveConnection, driveStatus, isSyncing, fileInputRef }) {
  ...
  const catalogStats = useMemo(() => {
    const stats = new Map(catalogosVisibles.map((cat) => [cat.id, { compradores: new Set(), skus: new Set() }]));
    const byKey = new Map();
  ...
  return <div>
    ...
    <div className="catalog-grid">
      {catalogosVisibles.map((cat) => {
        const stats = catalogStats.get(cat.id) || { compradores: new Set(), skus: new Set() };
          ...
          <Card className="catalog-card">
            ...
            <CardContent>
              <div className="catalog-head">
                <div>
                  <h3>{cat.nombre}</h3>
                  <p className="catalog-note">{stats.compradores.size} compradores - {stats.skus.size} SKU</p>
                  <p>{cat.canal}</p>
                </div>
                <span className={cat.estado === "Activo" ? "pill green" : "pill"}>{cat.estado}</span>
              </div>
              <p className="meta"><CalendarDays size={16}/>{cat.vigencia}</p>
              <p className="meta">{cat.notificaciones ? <Bell size={16}/> : <BellOff size={16}/>} {cat.notificaciones ? "Notificaciones activas" : "Notificaciones inactivas"}</p>
              <div className="catalog-actions">
                <Button className="full" onClick={() => { setCatalogoActivo(cat); setActive("promos"); }}>Trabajar catalogo</Button>
                <Button className="full" variant="outline" onClick={() => onOpenAvances?.(cat)}><ListChecks size={16}/> Gestion de Avances</Button>
              </div>
            </CardContent>
          </Card>
        </motion.div>;
