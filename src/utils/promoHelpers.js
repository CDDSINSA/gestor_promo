...
function getOfferOwnerKey({ usuarioId, compradorId } = {}) {
  return normalizeCanal(usuarioId || compradorId);
}

export function createDerivedOfferId({ actividadId, tipoPromo, grupoOferta, rowId, usuarioId, compradorId }) {
  ...
  if (isComplexPromoType(tipoPromo) && groupKey) return `OFE-${activityKey}-${typeKey}${ownerKey ? `-${ownerKey}` : ""}-${groupKey}`;
...
export function resolveOfferId(row, rowId, activityId, tipoPromo, grupoOferta) {
  ...
  return row.ofertaId || row.oferta_id || createDerivedOfferId({ actividadId: activityId, tipoPromo, grupoOferta, rowId, usuarioId });
}
