...
function createDerivedOfferId({ actividadId, tipoPromo, grupoOferta, rowId }) {
  ...
  if (isComplexType(tipoPromo) && groupKey) return `OFE-${activityKey}-${typeKey}-${groupKey}`;
...
function resolveOfferId(row, rowId, actividadId, tipoPromo, grupoOferta) {
  return row.oferta_id || row.ofertaId || createDerivedOfferId({ actividadId, tipoPromo, grupoOferta, rowId });
}
